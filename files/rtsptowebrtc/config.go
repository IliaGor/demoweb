...
// --------------------структура для изменения JSON-----------------
type myConfigST struct {
	Server  myServerST            `json:"server"`
	Streams myStreamST 			  `json:"streams"`
}
//ServerST struct
type myServerST struct {
	HTTPPort      string   `json:"http_port"`
	ICEServers    []string `json:"ice_servers"`
	ICEUsername   string   `json:"ice_username"`
	ICECredential string   `json:"ice_credential"`
...
//StreamST struct
type myStreamST struct {
	Empiria  EmpiriaST   `json:"Empiria"`
}
type EmpiriaST struct {
	URL          string `json:"url"`
	OnDemand     bool   `json:"on_demand"`
	IP           string `json:"ip"`
}
